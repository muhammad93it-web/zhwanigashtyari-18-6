...
<div class="space-y-5 animate-fade-in">
    ...
    <div class="card p-4">
        <form method="GET" class="grid grid-cols-2 lg:grid-cols-6 gap-3">
            ...
            <div class="col-span-2 lg:col-span-6 flex gap-2">
                ...
                <a href="{{ route('transactions.create') }}" class="btn-warning">+ مامەڵەی نوێ</a>
            </div>
        </form>
    </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div class="card p-3 flex items-center justify-between">
            <span class="text-xs text-slate-500">کۆی مامەڵەکان</span>
            <span class="text-lg font-bold text-slate-800">{{ number_format($totals->total_count) }}</span>
        </div>
        <div class="card p-3 flex items-center justify-between">
            <span class="text-xs text-slate-500">کۆی دۆلار</span>
            <span class="text-lg font-bold text-green-600">${{ number_format($totals->total_usd, 2) }}</span>
        </div>
        <div class="card p-3 flex items-center justify-between">
            <span class="text-xs text-slate-500">کۆی دینار</span>
            <span class="text-lg font-bold text-amber-500">{{ number_format($totals->total_iqd, 0) }} د.ع</span>
        </div>
    </div>
    @endif
            ...
            <table class="w-full text-sm">
                <thead>
                    <tr class="text-right text-xs text-slate-500 border-b border-slate-200">
                        <th class="px-4 py-3 font-semibold">ژمارەی مامەڵە</th>
                ...
                <tbody>
                    ...
                    <tr class="table-row">
                        ...
                        <td class="px-4 py-3">
                            <a href="{{ route('clients.show', $tx->client) }}" class="text-slate-800 font-medium hover:text-green-600 transition-colors">{{ $tx->client?->name }}</a>
                        </td>
                        <td class="px-4 py-3"><span class="{{ $tx->type_badge }}">{{ $tx->type_name }}</span></td>
                        <td class="px-4 py-3 text-slate-600 max-w-[200px] truncate" title="{{ $tx->description }}">{{ $tx->description }}</td>
                        ...
                        <td class="px-4 py-3">
                            <div class="flex items-center gap-2">
                                <a href="{{ route('transactions.show', $tx) }}" class="text-slate-400 hover:text-cyan-600 transition-colors" title="وردبوونەوە">
                                ...
                                <a href="{{ route('transactions.print', $tx) }}" target="_blank" class="text-slate-400 hover:text-cyan-600 transition-colors" title="چاپ">
                    ...
                    <tr>
                        <td colspan="9" class="px-4 py-12 text-center">
                            <div class="text-slate-300 mb-2">
                                <svg class="w-12 h-12 mx-auto" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M3 4a1 1 0 011-1h12a1 1 0 110 2H4a1 1 0 01-1-1zm0 4a1 1 0 011-1h12a1 1 0 110 2H4a1 1 0 01-1-1zm0 4a1 1 0 011-1h12a1 1 0 110 2H4a1 1 0 01-1-1z" clip-rule="evenodd"/></svg>
                            </div>
                            <p class="text-slate-500 text-sm">هیچ مامەڵەیەک نەدۆزرایەوە</p>
