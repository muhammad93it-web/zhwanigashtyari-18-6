...
<div class="flex items-center justify-between mb-4 flex-wrap gap-2">
    ...
    <div class="flex items-center gap-2 flex-wrap">
        <a href="{{ route('suppliers.statement-print', $supplier) }}" target="_blank" class="btn-info !px-3 !py-1.5">چاپ (A4)</a>
        <a href="{{ route('suppliers.statement-excel', $supplier) }}" class="btn-primary !px-3 !py-1.5">Excel</a>
        <a href="{{ route('suppliers.statement-word', $supplier) }}" class="btn-primary !px-3 !py-1.5">Word</a>
        ...
        <a href="{{ route('suppliers.statements') }}" class="btn-outline !px-3 !py-1.5">گەڕانەوە</a>
    </div>
</div>

<div class="card p-4 mb-4 grid grid-cols-1 sm:grid-cols-3 gap-4 text-sm">
    <div><div class="text-xs text-slate-400">مۆبایل</div><div class="font-semibold text-slate-800">{{ $supplier->phone ?: '—' }}</div></div>
    <div><div class="text-xs text-slate-400">دۆخ</div><div class="font-semibold text-slate-800">{{ $supplier->is_active ? 'چالاک' : 'ناچالاک' }}</div></div>
    @if($supplier->notes)<div><div class="text-xs text-slate-400">تێبینی</div><div class="font-semibold text-slate-800">{{ $supplier->notes }}</div></div>@endif
</div>
...
<div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-5">
    @foreach(['IQD' => 'دیناری عێراقی', 'USD' => 'دۆلاری ئەمریکی'] as $code => $label)
        ...
        <div class="card p-5">
            ...
            <div class="flex justify-between py-1 text-sm"><span class="text-slate-500">کۆی کڕین</span><span class="font-semibold text-slate-800">{{ $num($s['purchase']) }}</span></div>
                ...
                @forelse($transactions as $t)
                    ...
                    <tr class="table-row">
                        ...
                        <td class="px-4 py-3 font-semibold {{ $t->type == 'purchase' ? 'text-red-600' : 'text-green-700' }}">
                            {{ $t->type == 'purchase' ? '+' : '−' }}{{ $num($t->amount) }} {{ $tc }}
                        </td>
