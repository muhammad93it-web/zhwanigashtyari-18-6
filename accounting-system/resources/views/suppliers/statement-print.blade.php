...
<div class="page">
    <div class="head">
        <div class="brand">
            ...
            <div>
                ...
                <div class="sub">کەشف حساب</div>
            </div>
        </div>
        <div class="doc-meta">
            <div>کەس: <b>{{ $supplier->name }}</b></div>
            @if($supplier->phone)<div>مۆبایل: <b>{{ $supplier->phone }}</b></div>@endif
    ...
    <div class="summary">
        @foreach(['IQD' => 'دیناری عێراقی (د.ع)', 'USD' => 'دۆلاری ئەمریکی ($)'] as $code => $label)
            ...
            <div class="box">
                <h3>{{ $label }}</h3>
                <div class="row"><span>کۆی کڕین</span><span>{{ $num($s['purchase']) }}</span></div>
