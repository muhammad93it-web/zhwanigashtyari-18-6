...
@section('title', 'زیادکردنی قەرز')
@section('page-title', 'زیادکردنی قەرز')
@section('page-subtitle', 'تۆمارکردنی قەرزی نوێ')

@section('content')
<div class="flex items-center justify-between mb-4">
    <h2 class="text-base font-bold text-slate-800">قەرزی نوێ</h2>
    <a href="{{ route('debts.index') }}" class="btn-outline">گەڕانەوە</a>
</div>

<div class="card p-5 max-w-3xl">
    <form method="POST" action="{{ route('debts.store') }}" class="grid grid-cols-1 sm:grid-cols-2 gap-4">
        @csrf

        <div class="sm:col-span-2">
            <label class="label">ناوی کەس <span class="text-red-500">*</span></label>
            <input type="text" name="party_name" value="{{ old('party_name') }}" class="input-field" required>
        ...
        <div>
            ...
            <select name="direction" class="input-field">
                <option value="receivable" {{ old('direction') === 'receivable' ? 'selected' : '' }}>قەرزی لای خەڵک</option>
                <option value="payable" {{ old('direction') === 'payable' ? 'selected' : '' }}>قەرزی ئێمە</option>
            </select>
        ...
        <div>
            ...
            <input type="date" name="debt_date" value="{{ old('debt_date', date('Y-m-d')) }}" class="input-field" required>
        ...
        <div class="sm:col-span-2 flex gap-2">
            ...
            <a href="{{ route('debts.index') }}" class="btn-outline">پاشگەزبوونەوە</a>
        </div>
    </form>
</div>
@endsection
