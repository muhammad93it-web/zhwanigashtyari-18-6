...
@section('title', 'نووسراو نوێ')
@section('page-title', 'نووسراو نوێ')
@section('page-subtitle', 'دروستکردنی نووسراو')
...
<div class="max-w-3xl">
    ...
    <form method="POST" action="{{ route('documents.store') }}" class="card p-5 space-y-4">
        @csrf
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
                ...
                <input type="text" name="doc_type" value="{{ old('doc_type') }}" class="input-field" placeholder="مثال: داواکاری، ڕاپۆرت">
                @error('doc_type') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
            </div>
            <div>
                <label class="label">بۆ کێ</label>
                <input type="text" name="recipient" value="{{ old('recipient') }}" class="input-field">
        ...
        <div>
            ...
            <textarea name="body" rows="10" class="input-field">{{ old('body') }}</textarea>
            @error('body') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
        </div>

        <div>
            <label class="label">بەروار *</label>
            <input type="date" name="doc_date" value="{{ old('doc_date', date('Y-m-d')) }}" class="input-field" required>
        ...
        <div class="flex items-center gap-2 pt-2">
            ...
            <a href="{{ route('documents.index') }}" class="btn-outline">پاشگەزبوونەوە</a>
        </div>
    </form>
</div>
@endsection
